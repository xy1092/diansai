...
def parse_pid(line: str):
    ...
    try:
        return {
            ...
            "p": float(parts[9]), "i": float(parts[10]),
            "d": float(parts[11]), "raw_out": float(parts[12]),
        }
    except ValueError:
        return None
...
def parse_line(line: str):
    ...
    try:
        return {
            "kind": "line", "ts_ms": int(parts[1]), "bias": float(parts[2]),
...
def parse_app(line: str):
    ...
    try:
        return {
            ...
            "seg_cm": float(parts[9]), "mission_time_ms": int(parts[10]),
            "state_name": STATE_NAME.get(int(parts[3]), str(parts[3])),
        }
    except ValueError:
        return None
...
def parse_gain(line: str):
    ...
    if len(parts) < 5:
        return None
    try:
        return {"kind": "gain", "ch": parts[1], "kp": float(parts[2]),
                "ki": float(parts[3]), "kd": float(parts[4])}
    except ValueError:
        return None
...
def parse_cfg(line: str):
    ...
    if len(parts) < 5:
        return None
    try:
        return {"kind": "cfg", "param": parts[1], "value": float(parts[2]),
                "min": float(parts[3]), "max": float(parts[4])}
    except ValueError:
        return None
...
def parse_blackbox(line: str):
    ...
    try:
        vals = [
            ...
            float(parts[11]), float(parts[12]), int(parts[13]),
            float(parts[14]), float(parts[15]), int(parts[16]),
